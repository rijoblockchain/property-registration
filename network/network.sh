...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/registrar/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Registrar Identities"

    createRegistrar

    infoln "Creating Users Identities"
...
COMPOSE_FILE_BASE=docker/docker-compose-registration-net.yaml
